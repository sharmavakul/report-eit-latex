...
\begin{itemize}
    \item After the prerequisites such as awscli, detect-secrets, and ja, a flexible and lightweight command-line JSON processor is installed the AWS environment is configured using the command \textit{aws configure} \cite{30}.
\end{itemize}
\begin{itemize}
    \item Once the configuration is completed, Prowler is installed using the command \textit{git clone https://github.com/prowler-cloud/prowler} \cite{30}.
\end{itemize}
\begin{itemize}
    \item To start the assessment of the AWS account using prowler, the command \textit{./prowler} is executed \ref{fig:prowlercheckexecution}. When the assessment finishes, the assessment report is generated as shown in the figure \ref{fig:assessmentreport}.
\end{itemize}
